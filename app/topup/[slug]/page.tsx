import { notFound } from 'next/navigation'
import Image from 'next/image'
import Navbar from '@/components/layout/Navbar'
import Footer from '@/components/layout/Footer'
import TopUpClient from './TopUpClient'
import { GameDetail } from '@/lib/api'

interface Props {
  params: Promise<{ slug: string }>
}

async function getGame(slug: string): Promise<GameDetail | null> {
  const res = await fetch(`http://localhost:3000/api/games/${slug}`, { 
    cache: 'no-store'
  })
  if (!res.ok) return null
  const json = await res.json()
  return json.data
}

export default async function TopUpDetailPage({ params }: Props) {
  const { slug } = await params
  const game = await getGame(slug)
  if (!game) notFound()

  return (
    <div className="min-h-screen flex flex-col bg-white">
      <Navbar />

      {/* ── Banner Game ── */}
      {game.image_url ? (
        // Mode dengan image
        <div className="relative w-full overflow-hidden" style={{ minHeight: '220px' }}>
          <Image
            src={game.image_url}
            alt={game.name}
            fill
            className="object-cover"
            priority
          />
          {/* overlay gelap biar teks keliatan */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-black/10" />
          {/* decorative large text */}
          <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none select-none">
            <span
              className="font-black text-white/10 leading-none tracking-tight"
              style={{ fontSize: 'clamp(60px, 12vw, 140px)' }}
            >
              {game.name}
            </span>
          </div>
          {/* content */}
          <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-10">
            <div className="inline-flex items-center gap-1.5 text-white/80 text-xs font-semibold mb-3">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>
              </svg>
              Instant Delivery
            </div>
            <h1 className="text-3xl lg:text-4xl font-black text-white mb-1">{game.name}</h1>
            <p className="text-white/70 text-sm">{game.publisher} • {game.category}</p>
          </div>
        </div>
      ) : (
        // Mode tanpa image - background putih, teks hitam
        <div className="relative w-full overflow-hidden bg-gray-50 border-b border-gray-200" style={{ minHeight: '220px' }}>
          {/* decorative large text */}
          <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none select-none">
            <span
              className="font-black text-gray-200 leading-none tracking-tight"
              style={{ fontSize: 'clamp(60px, 12vw, 140px)' }}
            >
              {game.name}
            </span>
          </div>
          {/* content */}
          <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-10">
            <div className="inline-flex items-center gap-1.5 text-blue-600 text-xs font-semibold mb-3">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>
              </svg>
              Instant Delivery
            </div>
            <h1 className="text-3xl lg:text-4xl font-black text-gray-900 mb-1">{game.name}</h1>
            <p className="text-gray-500 text-sm">{game.publisher} • {game.category}</p>
          </div>
        </div>
      )}

      {/* ── Main Content ── */}
      <main className="flex-1 max-w-7xl mx-auto px-6 lg:px-8 py-10 w-full">
        <TopUpClient game={game} />
      </main>

      <Footer />
    </div>
  )
}

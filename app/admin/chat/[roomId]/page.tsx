...
import { connectChatSocket, joinRoom, leaveRoom, emitTyping, emitStopTyping, emitMarkRead } from '@/lib/chatSocket'
...
export default function AdminChatRoomPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        showError(err.message || 'Gagal memuat chat')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSendOffer = async () => {
    ...
    try {
      ...
      showError(err.message || 'Gagal mengirim penawaran')
    } finally {
      setSending(false)
    }
  }
  ...
  const handleRespondOffer = async (msgId: number, action: string) => {
    try {
      ...
      showSuccess(action === 'accepted' ? 'Penawaran diterima!' : 'Penawaran ditolak')
    } catch (err: any) {
      showError(err.message || 'Gagal merespon penawaran')
    }
  }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showError(err.message || 'Gagal mengirim gambar')
    } finally {
      setSending(false)
      ...
      <AdminShell>
        <div className="h-[70vh] flex flex-col items-center justify-center">
          <div className="flex flex-col items-center gap-4 bg-white border-[3px] border-gray-900 p-8 rounded-2xl shadow-[8px_8px_0px_#111827]">
            ...
            <p className="font-black text-gray-900 uppercase tracking-widest text-sm">Memuat Chat Admin...</p>
          </div>
        </div>
      </AdminShell>
    )
  ...
  return (
    <AdminShell>
      <div className="flex flex-col bg-white border-[3px] border-gray-900 rounded-2xl shadow-[8px_8px_0px_#111827] overflow-hidden h-[calc(100vh-120px)]">
        {/* ── Chat Header ── */}
        <header className="shrink-0 bg-gray-50 border-b-[3px] border-gray-900 p-4 flex items-center gap-3">
          {/* Back button */}
          <Link href="/admin/chat" className="w-10 h-10 flex items-center justify-center rounded-xl border-[3px] border-gray-900 bg-white hover:bg-gray-100 transition-colors shadow-[2px_2px_0px_#111827] shrink-0">
          ...
          <div className="relative shrink-0">
            <div className="w-11 h-11 rounded-xl border-[3px] border-gray-900 bg-gradient-to-br from-[#ffc900]/30 to-[#ff90e8]/30 flex items-center justify-center overflow-hidden shadow-[2px_2px_0px_#111827]">
              {other.avatar ? (
                <img src={resolveImageUrl(other.avatar)} className="w-full h-full object-cover" alt="" />
              ) : (
                <span className="text-base font-black text-gray-900">{other.name.charAt(0).toUpperCase()}</span>
              )}
            </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs font-bold text-gray-500 truncate">
              {otherTyping ? (
                <span className="text-blue-600 animate-pulse font-extrabold">Sedang mengetik...</span>
              ) : (
        ...
        {/* ── Sticky Product Card ── */}
        {room?.listing && (
          <div className="shrink-0 bg-yellow-50/50 border-b-[3px] border-gray-900 px-4 py-3">
            <div className="flex items-center gap-3">
              {/* Product image */}
              <div className="w-12 h-12 rounded-xl border-[3px] border-gray-900 overflow-hidden bg-white shrink-0 shadow-[2px_2px_0px_#111827]">
              ...
              <div className="flex-1 min-w-0">
                ...
                {room.listing.accountGame?.nama_game && (
                  <p className="text-[10px] sm:text-xs font-bold text-gray-500 truncate mt-0.5">
                    🎮 {room.listing.accountGame.nama_game}
                  </p>
                )}
              ...
              <div className="shrink-0">
                <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-green-100 border-2 border-green-600 rounded-lg">
                  ...
                  <span className="text-[10px] font-black text-green-700 uppercase tracking-wider">Tersedia</span>
                </span>
              </div>
            </div>
          </div>
        ...
        {/* ── Input Bar ── */}
        <div className="shrink-0 bg-white border-t-[3px] border-gray-900 p-4">
